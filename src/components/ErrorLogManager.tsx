          ...
          <table className="w-full text-xs text-white border-collapse">
            <thead>
              <tr className="bg-game-card border-b border-game-accent">
                <th className="text-left px-3 py-2 font-semibold text-gray-300 whitespace-nowrap">{t('errorLog.colWhen')}</th>
                <th className="text-left px-3 py-2 font-semibold text-gray-300 whitespace-nowrap">{t('errorLog.colUser')}</th>
                <th className="text-left px-3 py-2 font-semibold text-gray-300 whitespace-nowrap">{t('errorLog.colContext')}</th>
                <th className="text-left px-3 py-2 font-semibold text-gray-300">{t('errorLog.colMessage')}</th>
              </tr>
            </thead>
            <tbody>
              {logs.length === 0 && (
                <tr>
                  <td colSpan={4} className="text-center text-gray-500 py-6 italic">{t('errorLog.emptyNone')}</td>
                </tr>
              )}
              {logs.map((log) => (
                <tr key={log.id} className="border-b border-game-accent hover:bg-game-card/50 transition-colors">
                  ...
                  <td className="px-3 py-2 text-gray-300 whitespace-nowrap">{log.user_email ?? t('errorLog.unknownUser')}</td>
