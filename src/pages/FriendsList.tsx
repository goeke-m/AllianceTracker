...
export function FriendsList({ isAdmin }: FriendsListProps) {
  ...
  const [entries, setEntries] = useState<FriendsListEntry[]>([])
  ...
  async function load() {
    setLoading(true)
    const { data, error } = await supabase.from('friends_list').select('*')
    if (!error && data) setEntries(data as FriendsListEntry[])
    setLoading(false)
  }
  ...
  function openEdit(entry: FriendsListEntry) {
  ...
  async function handleSave() {
    ...
    const { error } = form.id
      ? await supabase.from('friends_list').update(payload).eq('id', form.id)
      : await supabase.from('friends_list').insert(payload)
    setSaving(false)
    if (error) {
  ...
  async function handleDelete(id: string) {
    setDeletingId(id)
    await supabase.from('friends_list').delete().eq('id', id)
    setDeletingId(null)
    load()
  }
  ...
  return (
    <div className="p-4 pb-24 space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-game-standard">
          🤝 {t('friends.title')} ({displayed.length}{displayed.length !== entries.length ? ` / ${entries.length}` : ''})
        </h1>
        {isAdmin && (
          <button
            onClick={openAdd}
              ...
              {displayed.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 4 : 3} className="text-center text-gray-500 py-6 italic">
                    {entries.length === 0 ? t('friends.emptyNoEntries') : t('common.emptyNoMatch')}
                  </td>
                </tr>
              )}
              {displayed.map(e => (
      ...
      {form && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-game-card border border-game-accent rounded-2xl w-full max-w-md p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-game-standard font-bold">{form.id ? t('common.edit') : t('common.add')} {t('friends.entryTitleSuffix')}</h2>
