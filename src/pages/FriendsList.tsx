    ...
    <div className="p-4 pb-24 space-y-3">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-game-standard">
          🤝 Friends List ({displayed.length}{displayed.length !== entries.length ? ` / ${entries.length}` : ''})
        </h1>
        {isAdmin && (
          <button
            onClick={openAdd}
              ...
              {displayed.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 4 : 3} className="text-center text-gray-500 py-6 italic">
                    {entries.length === 0 ? 'No entries on the friends list.' : 'No entries match the filter.'}
                  </td>
                </tr>
              )}
              {displayed.map(e => (
      ...
      {form && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
          <div className="bg-game-card border border-game-accent rounded-2xl w-full max-w-md p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-game-standard font-bold">{form.id ? 'Edit' : 'Add'} Friends List Entry</h2>
